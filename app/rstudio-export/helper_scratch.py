...
def igraph_connect_all_top_nodes_criss_cross(G, top_nodes):
    import itertools
    new_edge_list = list(itertools.combinations(top_nodes, 2))
    old_edge_list = [(e.source, e.target) for e in (G.es)]
    for each in new_edge_list:
        if not each in old_edge_list:
            if not (each[1], each[0]) in old_edge_list:
                G.add_edges([each])
...
def go_write_partial_a_and_b(G, write_head, s_d_pair_index, partial_results, write_increment):
    ...
    new_write_head = write_head + write_head
    G.es["sd_pairs"] = ""
    return new_write_head


def go_write_full_a_and_b(G, s_d_pair_index):
    import os
    dir_path = os.path.join(os.getcwd(), 'data')

    output_a_file = open(os.path.join(dir_path, 'a.csv'), "w")
    output_a_file.write(str(G.ecount()) + "," + str(s_d_pair_index) + "\n")
    b_file = open(os.path.join(dir_path, 'b.csv'), "w")

    for edge in G.es:
        # output_a_file.write(str(edge.index) + '\t' + edge["sd_pairs"][:-1] + "\n")
        output_a_file.write(edge["sd_pairs"][:-1] + "\n")
        b_file.write(edge["edge_traffic"] + "\n")
    output_a_file.close()
    b_file.close()
    G.es["sd_pairs"] = ""
